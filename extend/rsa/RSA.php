 ...
 */
     ...
     * @return string 密文
     */
    public function pri_encrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING)
    {
        $ret = false;
     ...
     * @return string 密文
     */
    public function pub_encrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING)
    {
        $ret = false;
        if (!$this->_checkPadding($padding, 'en')) $this->_error('padding error');
     ...
     * @return string 明文
     */
    public function pri_decrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING, $rev = false)
    {
        $ret = false;
     ...
     * @return string 明文
     */
    public function pub_decrypt($data, $code = 'base64', $padding = OPENSSL_PKCS1_PADDING, $rev = false)
    {
        $ret = false;
        $data = $this->_decode($data, $code);//base64解码
        if (!$this->_checkPadding($padding, 'de')) $this->_error('padding error');
        if ($data !== false) {
     ...
     * 生产新密匙
     * Power by Mikkle
     * rewrite by fjw in 19.7.30
     */
    public function buildNewKey($path = null, $name = null){
        ...
        $path = empty($path)?ROOT_PATH.config('rsa_file_path'):$path;
        $name = empty($name)?config('rsa_file_name'):$name;
        
        $pub = file_put_contents($path.$name.'.pub', $publicKey);
        $key = file_put_contents($path.$name.'.key', $privateKey);

        return ($pub > 0 && $key > 0)?true:false;
