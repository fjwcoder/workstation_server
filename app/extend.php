...
// +---------------------------------------------------------------------+
// | 重新封装数据传输RSA加解密方法  add by fjw in 19.7.30
// | 
// | 
// | 
// +---------------------------------------------------------------------+
// 获取秘钥文件
function rsa_file($filename = null){
    $path = ROOT_PATH.config('rsa_file_path').config('rsa_file_name');
    if(empty($filename)){
        return [
            'pub'=>$path.'.pub',
            'key'=>$path.'.key',
        ...
        return ($filename == 'pub')?$path.'.pub':$path.'.key';
    }
}
 ...
 *  rsa 加密
 *  @param data 加密数据
 *  @param type 加密方式（公钥pub  私钥pri）
 */
function rsa_encrypt($data, $type = 'pri'){

    $data = is_array($data)?json_encode($data, 320):$data;
    
    $path = ROOT_PATH.config('rsa_file_path').config('rsa_file_name');

    $rsaObj = new RSA($path.'.pub', $path.'.key');

    switch($type){
        case 'pri':
            return $rsaObj->pri_encrypt($data);
        break;
        case 'pub':
            return $rsaObj->pub_encrypt($data);
        break;
        default: 
            return false; 
        break;
 ...
 *  @param type 加密方式（公钥pub  私钥pri）
 */
function rsa_decrypt($data, $type = 'pri'){
    ...
    switch($type){
        case 'pri':
            return $rsaObj->pri_decrypt($data);
        break;
        case 'pub':
            return $rsaObj->pub_decrypt($data);
        break;
        default: 
            return false; 
        break;
...
function rsa_sign($data){
    ...
    return $rsaObj->sign($data);
}

// rsa 验证签名
function rsa_verity($data, $sign){
    ...
    return $rsaObj->verify($data, $sign);
}

// +---------------------------------------------------------------------+
// | 重新封装数据传输AES加解密方法  add by fjw in 19.7.30
// | 
// | 
// | 
// +---------------------------------------------------------------------+
