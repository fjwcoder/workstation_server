...
class Api extends LogicBase
{
    ...
    public function apiEdit($data = [])
    {
        ...
        // add by fjw in 19.7.31 ：增加编辑api接口后，设置缓存，不限期
        $is_cache = $this->modelApi->setCacheInfo()['is_cache'];
        $success_msg = $is_cache?'数据库+缓存成功':'数据库成功，缓存失败';

        return $result ? [RESULT_SUCCESS, $success_msg, $url] : [RESULT_ERROR, $this->modelApi->getError()];
    }
