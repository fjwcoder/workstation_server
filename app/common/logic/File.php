...
class File extends LogicBase
{
    ...
    public function pictureUpload($name = 'file', $thumb_config = ['small' => 100, 'medium' => 500, 'big' => 1000])
    {
        ...
        $picture_info = $this->modelObPicture->getInfo(['sha1' => $sha1], 'id,name,path,sha1');
        ...
        $result = $this->modelObPicture->setInfo($data);
    ...
    public function fileUpload($name = 'file')
    {
        ...
        $file_info = $this->modelObFile->getInfo(['sha1' => $sha1], 'id,name,path,sha1');
        ...
        $result = $this->modelObFile->setInfo($data);
    ...
    public function checkStorage($result = 0, $method = 'uploadPicture')
    {
        ...
        $method != 'uploadPicture' ? $this->modelObFile->setFieldValue(['id' => $result], 'url', $storage_result) : $this->modelObPicture->setFieldValue(['id' => $result], 'url', $storage_result);
    ...
    public function getPictureUrl($id = 0)
    {
        ...
        $info = $this->modelObPicture->getInfo(['id' => $id], 'path,url');
    ...
    public function getFileUrl($id = 0)
    {
        ...
        $info = $this->modelObFile->getInfo(['id' => $id], 'path,url');
    ...
    public function checkPictureExists($param = []) {
        return $this->modelObPicture->where('sha1',$param['sha1'])->find();
    }

    public function checkFileExists($param = []) {
        return $this->modelObFile->where('sha1',$param['sha1'])->find();
    }
}
